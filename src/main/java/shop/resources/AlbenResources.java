...
public class AlbenResources{
	...
	public Response postAlben(Album album, @Context UriInfo uriInfo) {

		// Product is saved with a new generated id, which is returned in the response location URI
		DatabaseReference ref = ShopService.albenRef.push();
		album.albumId = ref.getKey();
		ref.setValueAsync(album);

		URI uri = uriInfo.getAbsolutePathBuilder().path(album.albumId).build();

		return Response.created(uri).entity(album).build(); // 201
	}

	@Path("{id}")
	public AlbumResources getAlben(@PathParam("id") String id) {
		return new AlbumResources(id);
	}
